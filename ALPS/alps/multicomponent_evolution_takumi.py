...
def random_interaction_matrix(num_comp, chi_mean=None, chi_std=1, verbose=False):
...
def get_uniform_random_composition(num_phases: int, num_comps: int, verbose=True) -> np.ndarray:
    ...
    for n in range(num_phases):
        ...
        for d in range(num_comps):
            x = np.random.beta(1, num_comps - d) * phi_max # original
            # x = np.random.random() * phi_max
            phis[n, d] = x
            # Update the maximum value of phi_max
            phi_max -= x
    if verbose:
...
def evolve_dynamics(chis, phis_init, sort_phases=False, dt_tol=1e-7,
                    return_progress=False, inc=100):
    """evolve a particular system governed by a specific interaction matrix

    Args:
        chis: nd arr (ncomp, ncomp), The interaction matrix
        phis_init: nd arr (nphases, ncomp),  The initial composition of all phases
    ...
    if return_progress:
        phi_progress = [phis.copy()]
        t = [0]
    count = 0
    # run until convergence
    
    while not np.allclose(phis, phis_last, rtol=TOLERANCE, atol=TOLERANCE): # CHANGE THIS TO Delta mu < TOLERANCE
    # while not np.allclose(phis, phis_last, rtol=TOLERANCE, atol=TOLERANCE):
        ...
        while True:
            try:
                ...
                count += 1
                if return_progress and count % inc == 0:
                    t.append(t[-1] + dt)
                    if sort_phases:
                        phis_ = np.sort(phis, axis=0)[::-1]
                    else:
                        phis_ = phis.copy()
                    phi_progress.append(phis_)
            except RuntimeError or OverflowError as err:
                # If sim doesn't converge, try with reduced dt
                dt /= 2
                steps_inner *= 2
                phis[:] = phis_last
                ...
                if dt < dt_tol:
                    ...
                    print(f"evolve_dynamics: Reached minimal time step. dt={dt} Returning the most recent phis.")
                    break
                ...
                break
    if sort_phases:
        phis = np.sort(phis, axis=0)[::-1]

    if return_progress:
        t.append(t[-1] + dt) # Always add the last time point
        phi_progress.append(phis)
        # Volume fractions at the end of the simulation, intermediate volume fraction for each layer, and time
        return phis, phi_progress, t
    else:
        return phis
